...
def eval_fn(parameters):
    ...
    # The contact times are originally in range from 0 to 10.
    # the values are normalized by dividing by 5. This makes our range go from 0 to 2.
    # the qd algorithm will only use values from 0 to 1, so this way we discard the values above 1.
    # Values above 1 is the leg being in contact with the ground for over 50% of the time.
    contact_times = {k: v / 5 for k, v in contact_times.items()}
...
if __name__ == "__main__":
    ...
    grid = containers.Grid(shape=(10,10,10,10), max_items_per_bin=1, fitness_domain=((0., 80.),), features_domain=((0., 1.), (0., 1.), (0., 1.), (0., 1.)))
    algo = algorithms.RandomSearchMutPolyBounded(grid, budget=2500000, batch_size=512,
